        ...
        if (orderData.products.length > 0) {
          const data = await Promise.all(
            orderData.products.map(async (product) => {
              ...
              return productDetails ? { quantity: product.quantity, data: productDetails } : null;
            })
          );
    ...
    try {
      const response = await axios.post(
        'https://sneaker-production.up.railway.app/api/order/create',
        {
          ...
          totalAmt,
          payment_method,
          delivery_address, // Truyền userId từ localStorage
        },
        { withCredentials: true }
      );
      ...
      <Dialog open={openPayment} onClose={handleClose} className="relative">
        ...
        <DialogActions className="flex flex-col items-center space-y-4 p-4">
          <div className="flex flex-col items-center space-y-2">
            ...
            <img
              ...
              alt="QR Code"
              className="w-48 h-48 object-contain border-2 border-gray-300 rounded-md shadow-lg"
            />
          </div>
        </DialogActions>
        <DialogActions className="flex justify-center p-4">
          <Button
            onClick={handleClose}
            color="secondary"
            className="bg-red-500 hover:bg-red-600 text-white font-semibold rounded-full px-6 py-2"
          >
            Cancel
          </Button>
        </DialogActions>
